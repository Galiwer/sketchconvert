...
# Configure logging FIRST — before any local imports that use loggers at module level.
# NOTE: logging.basicConfig() is a no-op if uvicorn has already added handlers to the
# root logger. So we explicitly configure the 'backend' logger with its own handler
# and level, making it independent of whatever uvicorn sets on the root logger.
_log_level = getattr(logging, os.getenv("LOG_LEVEL", "INFO").upper(), logging.INFO)
_backend_logger = logging.getLogger("backend")
_backend_logger.setLevel(_log_level)
if not _backend_logger.handlers:
    _handler = logging.StreamHandler()
    _handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(name)s: %(message)s"))
    _backend_logger.addHandler(_handler)
    _backend_logger.propagate = False  # Don't double-log via root logger
...
def _validate_and_decode_image(image_b64: str,
                               max_bytes: int = int(os.getenv("MAX_IMAGE_BYTES", str(5 * 1024 * 1024)))):
    ...
    if "," in image_b64 and image_b64.startswith("data:image"):
        ...
        if not (header.startswith("data:image/png") or header.startswith("data:image/jpeg")):
            raise HTTPException(status_code=415,
                                detail=_error("unsupported_media_type", "Only PNG or JPEG data URLs are supported",
                                              header))
    ...
    if len(raw) > max_bytes:
        raise HTTPException(status_code=413, detail=_error("payload_too_large", f"Image too large (>{max_bytes} bytes)",
                                                           f"size={len(raw)}"))
    ...
    try:
        ...
        raise HTTPException(status_code=400,
                            detail=_error("invalid_image", "Decoded bytes are not a valid PNG/JPEG", str(e)))
...
def generate(data: Sketch):
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=_error("inference_error", "Internal error during inference", str(e)))
